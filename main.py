...
def handle_snap(message):
    ...
    if "snapchat.com" in url:
        ...
        try:
            # استخدام API بديل ومستقر للسناب (TikWM Extra)
            api_url = f"https://www.tikwm.com/api/extra/snapchat?url={url}"
            response = requests.get(api_url).json()
            
            if response.get('code') == 0 and response.get('data'):
                video_url = response['data'].get('url')
                bot.send_video(user_id, video_url, caption="✅ تم التحميل بنجاح | Downloaded Successfully")
                bot.delete_message(user_id, prog.message_id)
            else:
                bot.edit_message_text("❌ تأكد أن القصة عامة وليست خاصة\nMake sure the story is public", user_id, prog.message_id)
        except Exception:
